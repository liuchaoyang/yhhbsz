...
<div style="padding:5px;margin-left: 150px;">
	<div class="easyui-panel" title="2型糖尿病随访记录表" style="width:900px;">
	    <form id="mform" action="chrgly" method="post">
	    	...
	    	<table style="width:100%;" border="0"  cellpadding="0" cellspacing="0" class="data_table">
           		<tr >
           			<td  colspan="4" style="width:100%;" align="left" class="table_title">个人信息</td>
           		</tr>
  				<tr>
				...
				<tr>
					...
					<%-- <%if(opt==1){%> --%>
				      <tr>
				        ...
				        <td><label>每次</label><input class="easyui-numberbox input50" type="text" name="sportm" style="width:100px" value="" data-options="min:0,max:500"/>分钟</td>
				        <td><label>每周</label><input class="easyui-numberbox input50" type="text" name="sportt" style="width:100px" value="" data-options="min:0,max:50"/>次</td>
				      </tr>
				     <%--  <%}%> --%>
				    </table></td>
					 <td colspan="1" >
				...
				<tr><td>糖化血红蛋白(%)</td>
			        ...
			        <td>检查日期 </td>
			        <td width="100" colspan="1"><input name="zcxdate" class="Wdate" value="" onClick="WdatePicker({maxDate:'%y-%M-%d'})" style="height:25px;width:150px"/></td>
				</tr>
				<tr>
					<td colspan="3">
						...
						<%-- <%if(opt == 1){%>	 --%>
	  						<tr>
								...
								<td>检查日期 </td>
								<td><input name="zcxdate0" value="" class="Wdate" value="" onClick="WdatePicker({maxDate:'%y-%M-%d'})" style="height:25px;width:100px"/></td>
								<td height="30">检查结果</td>
								<td colspan="3"><textarea name="zcxresult0"  class="easyui-validatebox" style="height:30px;width:200px;" onkeyup="LimitTextArea(this,100)" onkeydown="LimitTextArea(this,100)"></textarea></td>
	 						</tr>
						<%-- <%}%> --%>
  						</table>
					</td>
	...
	<script> 
		...
		function addSportInput(fy,time){
			...
			var tesst = $("#sporthtml");
			sportlen+=1;
			tesst.append('<tr class="mainform"><td><input type="checkbox" name="ckbSport" ></td><td><label>每次</label><input class="easyui-numberbox input50" style="width:90px" type="text" name="sportm'+sportlen+'" value="'+fy+'" data-options="min:0,max:500"/>分钟</td><td><label>每周</label><input class="easyui-numberbox input50" style="width:90px" type="text" name="sportt'+sportlen+'" value="'+time+'" data-options="min:0,max:50"/>次</td><td></td></tr>');  
		}
		...
		function majaxSubmit(atname){
			$("#mform").form("submit", {
			url : "<%=request.getContextPath()%>/chr/glyInfo_doXTRecord.action",
			onSubmit : function() {
				return true;
			},
