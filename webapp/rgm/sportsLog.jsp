...
<head>
	...
	$(function(){
		archiveGrid = $("#archives").datagrid({
			...
			rownumbers: true,
			pagination: true,
			url:'<%=request.getContextPath()%>/rgm/sportLog_listSportLog.action',
	...
	function query(){
		...
		option.pageNumber = 1;
		option.url = "<%=request.getContextPath()%>/rgm/sportLog_listSportLog.action";
	...
	function editSportDetail(){
		var urls='<%=request.getContextPath()%>/rgm/sportLog_toEdit.action';
		top.addTab("运动详情",urls,null);
		}
	...
	function showLogDetail(){
		...
		if(!selections || selections.length != 1){
			...
			var url = "<%=request.getContextPath()%>/rgm/sportLog_showDetails.action?sportTime=" + selections[0].sportTime;
	       	var tabTitle = "日志详情";
